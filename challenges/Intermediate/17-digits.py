...
# You are expected to come up with a solution using the concept of recursion or the 
# so-called recursive approach.
# Check out the Resources for more topics about recursion to read on (if you aren't 
# familiar with it yet or haven't fully understood the concept behind it before taking 
# up this challenge or unless otherwise).
# An iterative version of this challenge can be found via this link.

def digits_count(num):
    if len(str(num)) == 1:
        return 1
    return 1 + digits_count(str(num)[1::])
